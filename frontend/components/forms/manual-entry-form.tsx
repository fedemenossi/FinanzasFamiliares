...
export function ManualEntryForm({ mode, categories = [], onCreated }: Props) {
  ...
  async function submit(formData: FormData) {
    ...
    try {
      ...
      if (mode === "income") {
        await createIncome({ income_date: new Date(`${date}T00:00:00`).toISOString(), description, amount, notes });
      } else {
        await createExpense({
